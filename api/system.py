...
td{{padding:8px 12px;border-top:1px solid #2a2d3a;font-size:14px}}
...
<h2 style="margin:24px 0 12px;font-size:16px">存储后端</h2>
<table>
<tr><th>类型</th><td>{storage_health.get('type','?')}</td></tr>
<tr><th>状态</th><td class="{'green' if storage_health.get('status')=='healthy' else 'red'}">{storage_health.get('status','?')}</td></tr>
</table>
<div class="refresh">最后更新: """ + stats_json.get("version", "") + """ · JSON: <span class="api-url">/health?format=json</span></div>
</div></body></html>""")
